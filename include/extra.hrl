-ifndef(N2O_EXTRA_HRL).
-define(N2O_EXTRA_HRL, true).
...
-record(upload_state, {dir, cid, root=code:priv_dir(n2o), name, type, room=upload, data= <<>>, preview=false, size=[{200,200}], index=0, block_size=1048576}).
-record(upload, {?ELEMENT_BASE(element_upload), name, value, state=#upload_state{}, root, delegate_query, delegate_api, post_write, img_tool, post_target, size, preview}).
...
-record(htmlbox, {?ELEMENT_BASE(element_htmlbox), html, state=#upload_state{}, root, delegate_query, delegate_api, post_write, img_tool, post_target, size}).
